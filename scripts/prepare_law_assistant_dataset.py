...
def prepare(input_dir: Path, output_dir: Path) -> dict[str, Any]:
    ...
    for row in pq.ParquetFile(input_paths["content"]).iter_batches(batch_size=5000, columns=["id", "content_html"]):
        ...
        for raw_id, raw_html in zip(ids, html_values):
            doc_id = int(raw_id)
    ...
    importer_metadata = metadata_clean[metadata.columns.tolist()]
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("data_usable"))
